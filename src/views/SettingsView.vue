          ...
          <div class="col-span-12 lg:col-span-2">
            ...
            <ul class="flex-col justify-start hidden p-0 m-0 list-none lg:flex">
              <li v-for="item in navItems" :key="item.name"
                :class="['-ml-[2px] border-l-2 transition-colors', activeName === item.name ? 'border-primary' : 'border-surface-200 dark:border-surface-700 hover:border-primary']">
                <RouterLink :to="{ name: item.name }"
                  :class="['flex items-center px-4 py-3 font-medium cursor-pointer', activeName === item.name ? 'text-primary' : 'text-surface-700 dark:text-surface-200 hover:text-primary']">
            ...
            <div class="lg:hidden">
              <div class="overflow-x-auto border-b border-surface-200 dark:border-surface-700">
                <ul class="flex flex-row justify-start p-0 m-0 list-none whitespace-nowrap">
                  <li v-for="item in navItems" :key="item.name" class="relative px-4 py-3 transition-colors"
                    :class="activeName === item.name ? 'text-primary' : 'text-surface-700 dark:text-surface-200 hover:text-primary'">
                    <RouterLink :to="{ name: item.name }" class="flex items-center font-medium cursor-pointer">
                      <i :class="[item.icon, 'mr-2', 'text-base!', 'leading-normal!']" />
                      <span>{{ item.label }}</span>
                    </RouterLink>
                    <div class="absolute bottom-0 left-0 right-0 h-[2px] transition-colors"
                      :class="activeName === item.name ? 'bg-primary' : 'bg-surface-200 dark:bg-surface-700'" />
                  </li>
                </ul>
              </div>
            </div>
